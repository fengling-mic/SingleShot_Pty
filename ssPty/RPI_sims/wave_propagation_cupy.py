...
def make_R_abs(X_in, Y_in):
    R_out = cp.empty_like(X_in)
    cp.abs(X_in, out=R_out)
    R_out = cp.maximum(R_out, cp.abs(Y_in))
    return R_out

def make_R_tri(X_in, Y_in):
    # Distances to three edges of an equilateral triangle
    theta = cp.mod(cp.arctan2(Y_in, X_in), 2*cp.pi/3)
    r = cp.sqrt(make_R(X_in, Y_in))
    R_out = r * cp.cos(theta - cp.pi/3) / cp.cos(cp.pi/3)
    return R_out
